...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair::<i32>("10,hello", ','), None);
...
#[test]
fn test_complex() {
    assert_eq!(parse_complex("1.5,2.1"), Some(Complex { re: 1.5, im: 2.1 }));
}
...
fn main() {
    // println!("Hello, world!");
    // escape_time(Complex { re: 1.5, im: 0.0 }, 10);
    let asdf = vec![0, 1, 2];
    println!("printing out vector {:#?}", asdf);
}
